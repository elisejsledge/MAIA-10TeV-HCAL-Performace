...
gPad.Update()
h_v2_eff_theta.GetPaintedGraph().GetYaxis().SetRangeUser(0,1)

leg = init_legend(.65, .20, .97, .35)
leg.AddEntry(h_v1_eff_theta, "10 TeV v1 jets", "LE")
leg.AddEntry(h_v2_eff_theta, "10 TeV v2 jets", "LE")
leg.AddEntry(h_3TeV_eff_theta, "3 TeV jets", "LE")
leg.Draw()

draw_title("R = 0.4 Anti-kt Jet Reconstruction Efficiency")
c2.SaveAs("comparisonPlots/eff_vs_theta.pdf")
...
h_v2_reso_E.SetLineColor(kBlue)
...
h_v2_reso_E.GetYaxis().SetRangeUser(-0,.3)
...
h_v2_reso_theta.SetLineColor(kBlue)
...
h_v2_reso_theta.GetYaxis().SetRangeUser(-0,.3)
...
leg.AddEntry(h_v2_reso_theta, "10 TeV v2 jets", "LE")
leg.AddEntry(h_3TeV_reso_theta, "3 TeV jets", "LE")
leg.Draw()

draw_title("Reconstructed Neutron Energy Resolution")
c4.SaveAs("comparisonPlots/reso_vs_theta.pdf")
...
h_v2_resp_E.SetLineColor(kBlue)
...
h_v2_resp_E.GetYaxis().SetRangeUser(-0,.3)
h_v2_resp_E.GetXaxis().SetTitleOffset(1.2)
h_v2_resp_E.GetYaxis().SetRangeUser(-0.7, 0.1)
...
h_v2_resp_theta.SetLineColor(kBlue)
...
#h_jet_resolution_vs_E.GetXaxis().SetRangeUser(0., 250.)
h_v2_resp_theta.GetYaxis().SetRangeUser(-0.7, 0.1)
...
leg.AddEntry(h_v2_resp_theta, "10 TeV v2 jets", "LE")
leg.AddEntry(h_3TeV_resp_theta, "3 TeV jets", "LE")
leg.Draw()

draw_title("Reconstructed Neutron Energy Resolution")
c6.SaveAs("comparisonPlots/resp_vs_theta.pdf")
